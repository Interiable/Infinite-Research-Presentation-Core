...
def warden_node(state: AgentState, config=None):
    ...
    try:
        ...
        **Output Format**:
        ...
        print(f"⚠️ Warden Node Critical Failure: {e}")
        log_night_audit("Warden", f"Critical Failure: {str(e)}")
        return {
            "shared_knowledge": f"--- WARDEN FAILED ---\n{state.get('shared_knowledge', '')}",
            "next": "SUPERVISOR",
            "sender": "Warden",
            "messages": [SystemMessage(content=f"Warden Critical Failure: {e}")]
        }
